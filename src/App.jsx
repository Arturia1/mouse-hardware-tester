...
const App = () => {
  ...
  // --- Estados de Verificação (Persistentes) ---
  ...
  const isAptoParaUso = tests.leftClick && tests.rightClick && tests.scrollOk;
  ...
  const connectMouse = async () => {
    try {
      ...
      if (devices.length > 0) {
        ...
        setDeviceInfo({
          // Se o firmware não retornar nome, define como Genérico
  ...
  useEffect(() => {
    ...
    const handleWheel = (e) => {
      ...
      // Calibração ideal entre 100 e 120
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-zinc-700 font-sans select-none">
      ...
      <div className="bg-zinc-200 w-full max-w-6xl rounded-[40px] p-10 shadow-2xl flex flex-col lg:flex-row gap-10 items-center border border-white/20">
        ...
        <div className="flex-1 w-full space-y-4">
          ...
          <div className="bg-zinc-300 p-6 rounded-3xl shadow-inner border border-zinc-400/20">
            ...
            <div className="flex justify-between items-center mb-3">
              <p className={`text-4xl font-black ${tests.scrollOk ? 'text-green-600' : 'text-zinc-700'}`}>
                {currentDelta} <span className="text-sm font-normal text-zinc-500">Delta</span>
              </p>
              <div className={`px-3 py-1 rounded-full text-[10px] font-bold ${tests.scrollOk ? 'bg-green-100 text-green-700 shadow-sm' : 'bg-zinc-400/20 text-zinc-500'}`}>
                {tests.scrollOk ? '✓ CALIBRADO' : 'AGUARDANDO'}
              </div>
            </div>
            {/* Mensagem de Instrução para o Técnico */}
        ...
        {/* VISUALIZADOR (ESTADO PERSISTENTE) */}
        <div className="flex-none flex flex-col items-center gap-6">
            <div 
              className={`w-56 h-80 rounded-[75px] border-4 relative overflow-hidden transition-all duration-700
              ${isAptoParaUso 
                ? 'bg-green-500 border-green-400 shadow-[0_0_100px_rgba(34,197,94,0.8)] scale-105' 
            ...
            >
                ...
                <div className="absolute top-12 left-1/2 -translate-x-1/2 w-8 h-14 bg-zinc-200 rounded-full border border-zinc-300 flex justify-center py-2 z-10 shadow-sm">
                   <div className={`w-2 h-full rounded-full transition-all duration-500 ${tests.scrollOk ? 'bg-green-600 scale-y-110 shadow-[0_0_10px_green]' : 'bg-zinc-400'}`} />
                </div>
            </div>
        ...
        {/* PERFORMANCE E RESET */}
        <div className="flex-1 w-full space-y-4">
          <div className="bg-zinc-800 p-8 rounded-3xl text-white shadow-2xl border-b-8 border-green-600">
             ...
             <div className="flex items-baseline justify-center gap-1">
               ...
               <span className="text-green-500 font-bold text-xl uppercase">Hz</span>
             </div>
          </div>
          ...
          <button 
            onClick={() => setTests({ leftClick: false, rightClick: false, scrollOk: false })} 
            className="w-full py-3 bg-zinc-400/10 text-zinc-500 hover:text-red-500 font-bold rounded-2xl transition-all text-xs uppercase"
          >
            Limpar Resultados
          </button>
        </div>
